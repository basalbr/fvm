        ...
        <form class="form-inline form-pesquisa">
            <div class="form-group" style="width: 200px">
                ...
                <select name="empresa" class="form-control">
                    ...
                    @foreach(\App\Pessoa::orderBy('nome_fantasia')->get() as $empresa)
            ...
            <div class="form-group" style="width: 130px">
                ...
                <input type="text" class="form-control date-mask" name='competencia_de'
                       value='{{Input::get('competencia_de')}}'/>
            </div>
            <div class="form-group" style="width: 130px">
                <label>Competência até</label>
                <input type="text" class="form-control date-mask" name='competencia_ate'
                       value='{{Input::get('competencia_ate')}}'/>
            </div>
            <div class="form-group" style="width: 130px">
                <label>Vencimento de</label>
                <input type="text" class="form-control date-mask" name='vencimento_de'
                       value='{{Input::get('vencimento_de')}}'/>
            </div>
            <div class="form-group" style="width: 130px">
                <label>Vencimento até</label>
                <input type="text" class="form-control date-mask" name='vencimento_ate'
                       value='{{Input::get('vencimento_ate')}}'/>
            </div>
            <div class="form-group" style="width: 160px">
                ...
                <select name="status" class="form-control">
                    ...
                    <option value="cancelado" {{Input::get('status') == 'cancelado' ? 'selected' : ''}}>Cancelado
                    </option>
                    <option value="concluido" {{Input::get('status') == 'concluido' ? 'selected' : ''}}>Concluído
                    </option>
            ...
            <div class="form-group" style="width: 220px">
                ...
                <select name="ordenar" class="form-control">
                    <option value="vencimento_desc" {{Input::get('ordenar') == 'atualizado_desc' ? 'selected' : ''}}>
                        Vencimento
                    </option>
                    <option value="competencia_desc" {{Input::get('ordenar') == 'competencia_desc' ? 'selected' : ''}}>
                        Competência
                    </option>
        ...
        <table class='table table-hover table-striped'>
            ...
            @if($processos->count())
                @foreach($processos as $processo)
                    <tr>
                        <td>
                            <a href="{{route('editar-empresa-admin', $processo->pessoa->id)}}">{{$processo->pessoa->nome_fantasia}}</a>
                        </td>
                        <td>{{$processo->imposto->nome}}</td>
                        <td>{{$processo->competencia_formatado()}}</td>
                        <td>{{$processo->vencimento_formatado()}}</td>
                        <td>{{$processo->getStatus()}}</td>
                        <td><a class='btn btn-primary'
                               href="{{route('visualizar-processo-admin', ['id' => $processo->id])}}"><span
                                        class="fa fa-search"></span> Visualizar</a></td>
                    </tr>
                @endforeach
            @else
                <tr>
